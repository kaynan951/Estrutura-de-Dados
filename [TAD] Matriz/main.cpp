...
int main()
{
	...
	while(true) {
		...
		cout << "Qual comando deseja realizar? " << endl;
		getline(cin, line);
		...
		// nlinhas [k]
			...
			int k;
			ss >> k;
			cout << "Numero de linhas: " << (matrizes[k])->linhas() << endl;
		}
        // ncolunas [k]
		else if(comando == "ncolunas") {
			int k;
			ss >> k;
			cout << "Numero de colunas: " << matrizes[k]->colunas() << endl;
		}
        // getvalor [i] [j] [k]
		...
		// sum [p] [q] 
			...
			if(C == nullptr) 
                cout << "nao foi possivel somar, sinto muito ;-;" << endl;
            else {
                C->print();
                delete C;
            }
        ...
        // multiply [p] [q] 
            ...
            int p, q;
			ss >> p >> q;
			Matriz *mult = matrizes[p]->multiplica(*(matrizes[q]));
			if(mult == nullptr) 
                cout << "nao foi possivel multiplicar, sinto muito ;-;" << endl;
            else {
                mult->print();
                delete mult;
            }
			...
			cout << "comando inexistente, tente novamente irmao \n" << endl;
		}
	}
	return 0;
}