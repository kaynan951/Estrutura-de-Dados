...
bool contains_recursion(Node* node, int value) {
    if (node == nullptr) {
        return false;
    }else if (node->key == value) {
        return true;
    } else
        return contains_recursion(node->left, value) || contains_recursion(node->right, value);
}
...
bool BinaryTree::contains(int value) const {
    return contains_recursion(root, value);
}

// função recursiva que imprime todas as chaves da arvore enraizada no ponteiro
...
// se voce passar a raiz de uma arvore para essa função, ela vai deletar todos os nos da arvore dada e retornar um ponteiro para nullptr(arvore vazia)
...
Node* serializeTree(stringstream &ss) {
    ...
    if(valor == "#") 
        ...
        int key = stoi(valor); // STOI =  string to int - tranforma o valor da string em inteiro
...
BinaryTree::BinaryTree(string serialization){
